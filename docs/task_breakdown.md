...
### ✅ Phase 6: Method Lineage Tracking Feature (Completed - 2025-11-10)

**Purpose**: Generate `method_lineage.csv` to easily track method evolution genealogy

**Components**:
- [x] **MethodTracker extension** (`analysis/method_tracker.py`)
  - `_global_block_id_map`: Map global_block_id across revisions
  - `to_tracking_format()`: Return traditional format (block_id + matched_block_id)
  - `to_lineage_format()`: Return lineage format (global_block_id, no matched_block_id)
  - Auto-assign global_block_id during tracking
  - 9 test cases passing

- [x] **CLI integration** (`commands/track.py`)
  - `--lineage` flag added
  - Auto-generate `method_lineage.csv`
...
**Usage Example**:
```bash
# Generate lineage format
b4-thesis track methods ./data -o ./output --lineage

# Output: method_tracking.csv + method_lineage.csv
```

**Benefits**:
- **Simple queries**: Same method = same `global_block_id`
- **Easy tracking**: No need to follow `matched_block_id`
- **Backward compatibility**: Traditional format still generated by default

**Test Status**: 11 tests passing (MethodTrackerLineage: 9, TrackMethods CLI: 2)

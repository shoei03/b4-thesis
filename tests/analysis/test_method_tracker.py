...
class TestMethodTrackerRevisionPair:
    ...
    def test_deleted_method_detected(self, tracker):
        ...
        # block_c should be deleted in rev2
        # Check if we track deletions - they might appear in rev1 with state "deleted"
        # or not appear at all. Let's check rev1 blocks
        rev1_blocks = result[result["revision"] == "20250101_100000_hash1"]
        block_c = rev1_blocks[rev1_blocks["block_id"] == "block_c"]
        if len(block_c) > 0:
            # If we track it, it should have matched_block_id as None
            assert block_c.iloc[0]["matched_block_id"] is None or pd.isna(
                block_c.iloc[0]["matched_block_id"]
            )
